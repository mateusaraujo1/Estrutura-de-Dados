...
int numFolhas(RB_node *t, int *count) {
    if (t) {
        numFolhas(t->left, count);
        numFolhas(t->right, count);
    	if (t && ((t->right == NULL) && (t->left == NULL)))
			*count += 1;
	}
	return *count;
}
...
int main()
{
	...
	int count = 0;
	printf("\nO numero de folhas eh: %d", numFolhas(r->root, &count));
