...
void printAVL(AVL *t) {
    ...
    //apenas pra poder separar a árvore da raíz, e poder usar a recursividade no printTreeInt sem zerar a height toda vez (ajuda quando a folha é NULL)
    //usuário passa apenas um parâmetro
}
...
//gcc AVL.c avl-main.c -c && gcc AVL.o avl-main.o && a
...
int main() {
    ...
    printf("\n");
    printAVL(treeInt); //modifiquei, antes passava a raíz como parâmetro, e agr é a árvore
    printf("\n");
    printf("%d", AVL_height(treeInt, &num[5])); //a raíz é zero
    /*
    int n = 11;
